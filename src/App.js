...
import {ListaTareas} from './componentes/InputLista';
...
function App() {
  ...
  const cargarMasElementos = () => {
    setElementosPorPagina(elementosPorPagina + 5)};
  ...
  // const ultimaAve = useCallback((nodo) => {
  //   console.log(nodo)
  //   if (nodo) {
  //     observador.lastChild = new IntersectionObserver((entrada) => {
  //       if (entrada.isIntersecting) {
  //         console.log('se ve'); // Aquí se carga más elementos cuando el último elemento está intersectando
  //       }
  //     });
      
  //     observador.lastChild.observe(nodo);
  //   }
  // },[carga,hayMas]);
  ...
  return (
    <><div className="App">
      ...
      <div className='contenedor-pokeSearch'>
        ...
        <div>{(!busqueda && carga) ? carga : (carga && "Cargando...")}</div>{/* {aparecemientras no haya nada en el input} */}
        <div>{error && 'error'}</div>
      </div>
