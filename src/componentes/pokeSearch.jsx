...
export function PokeSearch(busqueda, numeroPagina) {//funcion con 2 parametros
  ...
  useEffect(()=>{//use effect para uso de JS antes de return
    ...
    const cancelar = new AbortController();//nuevo metodo, linea anterior deprecated
    axios({//libreria actualizada de lazyLoad
      ...
      if(!busqueda){
        setPokes([])
      }else{setPokes([...new Set(res.data.birds.flatMap(b => b.members.map(member => member.toString().split(','))).flat())]);
    ...
    cancelar.abort()//nuevo metodo, junto con AbortController(), la posterior esta deprecated
    // return () => cancel()
